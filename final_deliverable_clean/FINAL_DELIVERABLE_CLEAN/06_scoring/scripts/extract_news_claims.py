...
def extract_news_claims(root: Path = SCORING_ROOT, limit: int | None = None) -> dict[str, Any]:
    ensure_directories(root)
    output_dir = root / "10_claims" / "news_claims"
    output_dir.mkdir(parents=True, exist_ok=True)
    rows = [row for row in read_csv_rows(registry_path("news", root)) if row.get("include_flag", "").strip().lower() in {"yes", "y", "true", "1"}]
    ...
    for row in rows:
        ...
        claims = extract_claims_from_text("news", document_id, text, row)
        for claim in claims:
    ...
    write_csv_rows(root / "10_claims" / "news_claims_long.csv", CLAIM_FIELDS, all_claim_rows)
    summary = {
        "source_type": "news",
    ...
    save_json(root / "10_claim_extraction" / "news_claim_extraction_summary.json", summary)
    return summary


def main() -> int:
    parser = argparse.ArgumentParser(description="Extract claim-level news statements.")
    parser.add_argument("--root", type=Path, default=SCORING_ROOT)
    parser.add_argument("--limit", type=int)
    args = parser.parse_args()
    print(json.dumps(extract_news_claims(args.root, args.limit), indent=2, ensure_ascii=False))
    return 0
